...
class HermesToolParser(ToolParser):
    ...
    def _get_arguments_config(self, func_name: str, tools: Optional[list[OpenAIFunctionToolSchema]]) -> dict:
        if not tools:
            return {}
    ...
    def _convert_param_value(self, param_value: str, param_name: str, param_config: dict, func_name: str) -> Any:
        ...
        if param_type in ["boolean", "bool", "binary"]:
            ...
            logger.warning(
                ...
                f"boolean (`true` or `false`) in tool '{func_name}', degenerating to string."
            )
            return param_value
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    ...
                    f"'{func_name}', will try Python literal parsing."
                )
        ...
        try:
            ...
            logger.warning(
                ...
                f"via Python literal parsing in tool '{func_name}', degenerating to string."
            )
            return param_value
    ...
    def _parse_legacy_tool_call(self, match: str) -> FunctionCall:
    ...
    async def extract_tool_calls(
        ...
        for match in matches:
            try:
                ...
                if stripped_match.startswith("<function="):
                    function_calls.append(self._parse_xml_function_call(stripped_match, tools))
                else:
                    function_calls.append(self._parse_legacy_tool_call(stripped_match))
