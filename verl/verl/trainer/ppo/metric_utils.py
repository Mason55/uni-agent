...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if "traj_masked" in batch.non_tensor_batch:
        traj_masked = batch.non_tensor_batch["traj_masked"].astype(np.float32)
        metrics["traj_masked/ratio"] = traj_masked.mean().item()
