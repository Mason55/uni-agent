"""Anthropic Messages API conversion for the training gateway.

The gateway stores conversations in OpenAI Chat Completions message format so
chat templates, prefix checks, and trajectory accounting share one path. This
module converts Anthropic ``/v1/messages`` requests into that internal shape
and converts gateway chat-completion responses back to Anthropic envelopes.
"""
